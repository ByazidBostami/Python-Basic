...
def add_student():
    ...
    with open("students.txt", "a") as file:
        file.write(f"{student_id},{first_name},{last_name},{current_year},{major},{first_name.lower()}.{last_name.lower()}@lut.fi\n")
...
def search_student():
    ...
    matching_students = [student_line for student_line in student_lines if search_term in student_line]
    if matching_students:
        print("Matching students:")
        for student_line in matching_students:
...
def search_course():
    ...
    matching_courses = [course_line for course_line in course_lines if search_term in course_line]
    if matching_courses:
        print("Matching courses:")
        for course_line in matching_courses:
            course_info = course_line.strip().split(",")
    ...
    for i, line in enumerate(passed_courses):
        if line.startswith(course_id + "," + student_id):
            ...
            with open("passed.txt", "w") as file:
                file.writelines(passed_courses)
            print("Record updated!")
            return

    with open("passed.txt", "a") as file:
        file.write(f"{course_id},{student_id},{date_str},{grade}\n")
    print("Record added!")
...
def show_student_record():
    ...
    student_info = [s_line.strip().split(",") for s_line in student_lines if student_id in s_line][0]
    
    with open("passed.txt", "r") as file:
        passed_courses = file.readlines()

    with open("courses.txt", "r") as file:
        courses_info = file.readlines()
    ...
    for line in passed_courses:
        if line.startswith(student_id):
            ...
            date = datetime.datetime.strptime(date_str, "%d/%m/%Y")
            course_info = [c_line.strip().split(",") for c_line in courses_info if course_id in c_line][0]
